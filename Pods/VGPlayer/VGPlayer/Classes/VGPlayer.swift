...
open class VGPlayer: NSObject {
    ...
    open var gravityMode : VGVideoGravityMode =  .resize
    ...
    //vishal
    open fileprivate(set) var player : AVQueuePlayer? {
        willSet{
            removePlayerObservers()
        }
        didSet {
    ...
    internal func configurationPlayer(_ URL: URL) {
        ...
        //print(#function,self.playerAsset,URL)
        if URL.absoluteString.hasPrefix("file:///") {
            let keys = ["tracks", "playable"];
            playerItem = AVPlayerItem(asset: playerAsset!, automaticallyLoadedAssetKeys: keys)
        } else {
            playerItem = resourceLoaderManager.playerItem(URL) //HLS - comment this and uncomment following
            //let keys = ["tracks", "playable"];
            //playerItem = AVPlayerItem(asset: playerAsset!, automaticallyLoadedAssetKeys: keys)
        }
        do {
          ...
          //  player = AVPlayer(playerItem: playerItem)
            player = AVQueuePlayer(playerItem: playerItem) //vishal //HLS
        } catch _ {
            //print("video Playing ")
        }
