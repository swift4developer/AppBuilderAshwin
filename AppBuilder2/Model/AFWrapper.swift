...
class AFWrapper: NSObject {
    ...
    class func requestPostURLForUploadAudio(_ strURL : String,isAudioSelect : Bool,fileName : String, params : [String : AnyObject]?,audioURL : URL, success:@escaping (Dictionary<String,Any>) -> Void, failure:@escaping (Error) -> Void){
        var audioName = ""
        let timeStamp = NSNumber(value: Date().timeIntervalSince1970)
        audioName = fileName + "." +  String(describing: timeStamp) + ".mp3"
        
        Alamofire.upload(multipartFormData: { multipartFormData in
            if isAudioSelect == true{
                let audioData = try? Data(contentsOf: audioURL)
                multipartFormData.append(audioData!, withName: fileName, fileName: audioName, mimeType: "mp3")
            }
            for (key, value) in params! {
                multipartFormData.append(value.data(using: String.Encoding.utf8.rawValue)!, withName: key)
            }
