...
class BlogHelpViewController: UIViewController,AVAudioPlayerDelegate,NVActivityIndicatorViewable {
   ...
   // @IBOutlet weak var slider: UISlider!
    ...
    func loadData(){
        ...
        if flagUI == "4"{
            ...
            //webViewMore?.topAnchor.constraint(equalTo: btnTopSignMe.bottomAnchor).isActive = true
            webViewMore?.topAnchor.constraint(equalTo: btnTopSignMe.bottomAnchor, constant: 7.5).isActive = true
        }
        
        webViewMore?.widthAnchor.constraint(equalTo: viewInsideScrollView.widthAnchor).isActive = true
        // webViewMore?.bottomAnchor.constraint(equalTo: btnBottomSignMe.topAnchor).isActive = true
        ...
        case "2":
            ...
            //bottomOfBtmSignupBtn.isActive = false
            //bottomOfBtmSignupBtn.constant = 80
            
            videoBottonConstraint.isActive = true
            //viewOfVideoPlayer.bottomAnchor.constraint(equalTo: btnTopSignMe.topAnchor).isActive = true
...
//                       animations: {
//                        self.viewOfBottom.center.y += self.viewOfBottom.bounds.height
//                        self.viewOfBottom.layoutIfNeeded()
//
...
//        })
    ...
    @IBAction func btnQuestionMarkClick(_ sender: Any) {
        if self.appDelegate.helpData.count > 0 && UserDefaults.standard.bool(forKey: "helpData"){
            player1.cleanPlayer()
            self.dicMediaData = self.appDelegate.helpData
            self.flgToShow = "Help"
            callData()
        }else {
                ...
                if let common_element = data["common_element"] as? [String:Any] {
                    ...
                    if data.keys.contains("articles"){
                        ...
                        if let top_buttonDic = artileElement["top_button"] as? Dictionary<String,String> {
                            ...
                            //btnTopSignMe.titleLabel?.textColor = UIColor().HexToColor(hexString: txtcolorHex!)
                        ...
                        if let bottom_buttonDic = artileElement["bottom_button"] as? Dictionary<String,String> {
                            ...
                            let sizeInt:Int = Int(size!)!
                            let bgColorBottomBtn = bottom_buttonDic["bgcolor"]
                            ...
                            //btnBottomSignMe.titleLabel?.textColor = UIColor().HexToColor(hexString: txtcolorHex!)
    ...
    @IBAction func topSignMeBtnClk(_ sender: UIButton) {
        ...
        if validation.isValidEmail(testEmail: url){
            ...
            if let requestUrl = NSURL(string: url) {
                ...
                let alrtTitleStr = NSMutableAttributedString(string: "Sorry!")
    ...
    @IBAction func bottomSignMeBtnClk(_ sender: UIButton) {
        print("BottomSignMeBtnClk click")
        let url = btn2Link
...
//                player.play()
    ...
    func videoAudioPlayer(urlStr: String, StrstartTime: String){
        ...
        //let url = URL(string:"https://membrandt-client-app-26.s3.us-west-1.amazonaws.com/course-files/videos/z64b3l-indian-cricket-team-respect-sportsmanship-fairplay-moments.mp4")
        self.player1.replaceVideo(url!)
...
extension BlogHelpViewController :WKUIDelegate, WKNavigationDelegate ,UIScrollViewDelegate{
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        //let blueColor = UIColor(red: 22.0/255.0, green: 121.0/255.0, blue: 225.0/255.0, alpha: 1.0)
       // btnBottomSignMe.backgroundColor = blueColor
       
        self.webViewMore?.evaluateJavaScript("document.readyState", completionHandler: { (complete, error) in
            if complete != nil {
                self.webViewMore?.evaluateJavaScript("document.body.offsetHeight", completionHandler: { (height, error) in
...
//MARK:- Auido Delegate
extension BlogHelpViewController {
                ...
                DispatchQueue.main.async {
                    ...
                    if let data = JSONResponse["response"] as? [String:Any] {
                        ...
                        if let strContent = self.dicMediaData.value(forKey: "contenturl") {
                            ...
                            print("strValue ::",strValue)
                            if strValue.count > 0 {
                                self.webViewMore?.loadUrl(string: (self.dicMediaData["contenturl"])! as! String)
                             }
                        }
                        self.loadData()
                    }
...
//MARK:- favMenuDelagte
extension BlogHelpViewController : favMenuDelagte {
    func homeBtnPress() {
...
extension BlogHelpViewController: VGPlayerDelegate {
    func updateTime() {
        // Access current item
...
extension BlogHelpViewController: VGPlayerViewDelegate {
